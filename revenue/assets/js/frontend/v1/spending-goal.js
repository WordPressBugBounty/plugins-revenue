				...
				if ('gift' === offer?.reward_type) {
					...
					if (accumulatedGoal > cartTotal) {
						...
						message = wp.i18n.sprintf(
							wp.i18n.__('Spend %s more to claim your gift!', 'revenue'),
							formattedAmount
						);
					} else if (quantity == 1) {
						message = wp.i18n.sprintf(
							wp.i18n.__('Spend %1$s more to get any %2$d item as a gift!', 'revenue'),
							formattedAmount,
							quantity
						);
					} else {
						message = wp.i18n.sprintf(
							wp.i18n.__('Spend %1$s more to get any %2$d items as a gift!', 'revenue'),
							formattedAmount,
							quantity
						);
					}
				} else if (isAll) {
					message = wp.i18n.__('Congrats! Your gift is here!', 'revenue');
				} else if (quantity == 1) {
					message = wp.i18n.sprintf(
						wp.i18n.__('Congrats! Choose any %d item.', 'revenue'),
						quantity
					);
				} else {
					message = wp.i18n.sprintf(
						wp.i18n.__('Congrats! Choose any %d items.', 'revenue'),
						quantity
					);
					campaignId,
